...
static void test_save_load_roundtrip(void **state) {
    ...
    g_state.Round_num = 3;
    g_state.Winner_id = 1;
    g_state.Game_timer = 9999;
    g_state.My_char[0] = 5;
    g_state.My_char[1] = 10;
    g_state.Score[0][0] = 123456;
    g_state.Score[1][0] = 654321;
    ...
    // Verify restoration
    assert_int_equal(g_state.Round_num, 3);
    assert_int_equal(g_state.Winner_id, 1);
    assert_int_equal(g_state.Game_timer, 9999);
    assert_int_equal(g_state.My_char[0], 5);
    assert_int_equal(g_state.My_char[1], 10);
    assert_int_equal(g_state.Score[0][0], 123456);
    assert_int_equal(g_state.Score[1][0], 654321);
}
...
static void test_load_null_safety(void **state) {
    ...
    // Setup a known state
    g_state.Round_num = 42;

    // This should not crash and should not modify globals
    GameState_Load(NULL);

    // Verify state unchanged
    assert_int_equal(g_state.Round_num, 42);
}
