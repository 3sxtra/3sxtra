...
/* Task 5: HandleMenuExit when netplay is not active (IDLE state).
   Calling it in IDLE must not crash; the state may stay IDLE or move
   briefly to EXITING (both are valid — EXITING → IDLE on next Run). */
static void test_handle_menu_exit_not_connected(void **state) {
    (void) state;
    ...
    int s = Netplay_GetSessionState();
    assert_true(s == NETPLAY_SESSION_IDLE || s == NETPLAY_SESSION_EXITING);

    /* Restore to clean IDLE */
    reset_netplay_state();
}
...
int main(void) {
    const struct CMUnitTest tests[] = {
        ...
        /* Task 5 addition */
        cmocka_unit_test(test_handle_menu_exit_not_connected),
    };
