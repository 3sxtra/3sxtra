...
static Sint8 tr_characters[2] = { -1, -1 };
static Sint8 tr_super_arts[2] = { -1, -1 };
...
static void parse_inputs(int start_frame) {
    ...
    for (int i = 0; i < REPLAY_FRAMES_MAX; i++) {
        const int frame = start_frame + i;
        const Sint64 p1_offset = frame * 10;
        const Sint64 p2_offset = p1_offset + 5;
        inputs[i][0] = read_input_buff(io, p1_offset);
        inputs[i][1] = read_input_buff(io, p2_offset);
    }
}
...
static void initialize_data() {
    ...
    for (int frame_num = 0;; frame_num++) {
        ...
        if (in_game && !did_set_char_data) {
            ...
            SDL_ReadIO(io, tr_characters, 2);

            SDL_SeekIO(io, SUPER_ARTS_OFFSET, SDL_IO_SEEK_SET);
            SDL_ReadIO(io, tr_super_arts, 2);

            did_set_char_data = true;
        }

        // Leaving this here to parse inputs from states directly once I figure out CPS3 button mapping

        // if (in_game) {
        //     inputs[inputs_index][0] = read_u16(io, P1SW_OFFSET);
        //     inputs[inputs_index][1] = read_u16(io, P2SW_OFFSET);
        //     inputs_index += 1;
    ...
    for (int i = 0; i < 2; i++) {
        if (tr_characters[i] > CHAR_AKUMA) {
            tr_characters[i] -= 1;
        }
    }

    parse_inputs(game_start_frame);
}
...
void TestRunner_Prologue() {
    ...
    case PHASE_MENU:
        if (G_No[1] == 1 && G_No[2] == 2) {
            Last_My_char2[0] = tr_characters[0];
            Last_My_char2[1] = tr_characters[1];
            Last_Super_Arts[0] = tr_super_arts[0];
            Last_Super_Arts[1] = tr_super_arts[1];
    ...
    case PHASE_CHARACTER_SELECT:
        ...
        case 0:
            set_cursor(tr_characters[0], 0);
            set_cursor(tr_characters[1], 1);
