 ...
 * MENU_SCREEN_DEMO.
 *
 * Two demo flows exist, selected by g_state.D_No[0] at the time Play_Demo()
 * is called:
 *   g_state.D_No[0]==0  →  Demo00 path (quick start: gameplay runs until timeout)
 *   g_state.D_No[0]==1  →  Demo01 path (full attract: char select then gameplay)
 *
 * The callbacks delegate to the existing Demo00() / Demo01() functions
 * which manage their own phase state via g_state.D_No[]. The MenuScreen layer
...
static void ms_demo_tick(struct _TASK* tp) {
    /*
     * Delegate to the legacy demo functions. They manage their own g_state.D_No[]
