 ...
 *
 * L/R value changes are live — they modify g_state.Convert_Buff[3][1] directly via
...
static void sound_test_enter(struct _TASK* task_ptr) {
    ...
    task_ptr->timer = 5;
    setupAlwaysSeamlessFlag(((g_state.plsw_00[0] | g_state.plsw_00[1]) & 0x4000) != 0);
    Clear_Flash_Init(4);
    Menu_Common_Init();
    ...
    g_state.Convert_Buff[3][1][5] = 0;

    /* Init sound mode convert buffers */
    if (sys_w.sound_mode == 0) {
        g_state.Convert_Buff[3][1][0] = 0;
    } else {
        g_state.Convert_Buff[3][1][0] = 1;
    }

    if (sys_w.bgm_type == BGM_ARRANGED) {
        g_state.Convert_Buff[3][1][3] = 0;
    } else {
        g_state.Convert_Buff[3][1][3] = 1;
    }

    g_state.Convert_Buff[3][1][7] = 1;
    ...
    if (use_rmlui && rmlui_menu_sound) {
        ...
        g_state.Order[0x72] = 1;
        g_state.Order_Dir[0x72] = 8;
        g_state.Order_Timer[0x72] = 1;
...
static void sound_test_tick(struct _TASK* task_ptr) {
    ...
    last_mode = g_state.Convert_Buff[3][1][0];
    Sound_Cursor_Sub(0);
    ...
    if ((g_state.Menu_Cursor_Y[0] == 4) && (g_state.IO_Result == 0x100)) {
        SE_selected();
        g_state.Convert_Buff[3][1][0] = 0;
        g_state.Convert_Buff[3][1][1] = 0xF;
        g_state.Convert_Buff[3][1][2] = 0xF;
        g_state.Convert_Buff[3][1][3] = 0;
    }

    /* Live-apply BGM level */
    if (bgm_level != (s16)g_state.Convert_Buff[3][1][1]) {
        bgm_level = g_state.Convert_Buff[3][1][1];
        CurrentSave()->BGM_Level = g_state.Convert_Buff[3][1][1];
        SsBgmHalfVolume(0);
    }

    /* Live-apply SE level */
    if (se_level != (s16)g_state.Convert_Buff[3][1][2]) {
        se_level = g_state.Convert_Buff[3][1][2];
        setSeVolume(CurrentSave()->SE_Level = g_state.Convert_Buff[3][1][2]);
    }

    /* Persist BGM type */
    CurrentSave()->BgmType = g_state.Convert_Buff[3][1][3];

    /* Handle BGM type change — switch tracks */
    if (sys_w.bgm_type != g_state.Convert_Buff[3][1][3]) {
        sys_w.bgm_type = g_state.Convert_Buff[3][1][3];
        g_state.Convert_Buff[3][1][5] = 0;
        BGM_Request_Code_Check(0x41);
    }

    g_state.Order_Dir[0x7B] = g_state.Convert_Buff[3][1][5];
    ...
    if (g_state.Menu_Cursor_Y[0] == 5) {
        if (g_state.IO_Result == 0x100) {
            SsRequest((u16)g_state.Order_Dir[0x7B] + 1);
            g_state.Convert_Buff[3][1][7] = 1;
            return;
        }

        if ((g_state.IO_Result == 0x200) && g_state.Convert_Buff[3][1][7]) {
            g_state.Convert_Buff[3][1][7] = 0;
            BGM_Stop();
            return;
        }
    }
    ...
    if (g_state.IO_Result == 0x200 ||
        ...
        g_state.Order[0x72] = 4;
        g_state.Order_Timer[0x72] = 4;
        BGM_Request_Code_Check(0x41);
