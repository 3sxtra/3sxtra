...
#define CANVAS_W 384
#define CANVAS_H 224
#define CANVAS_PITCH 384
#define QUADS_MAX 8192

#if defined(CRS_SW_CANVAS_16BPP)
// 16bpp mode forces ckey_ok fast path on scaled blits too
#define CRS_SW_SCALED_CKEY 1
#endif

// Discard fullscreen fades and background solid quads that are entirely
...
static SWTexture textures[FL_TEXTURE_MAX];
static SWPalette palettes[FL_PALETTE_MAX];
static SWTextureSpec latest_texture_spec;
static SWQuad* quads = NULL; // stb_ds
...
static uint32_t ckey_pal_version = 0;
static const SWPalette* ckey_pal_src = NULL;

static const uint32_t* ckey_pal_get(const SWPalette* p) {
    if (ckey_pal_src != p || ckey_pal_version != p->version) {
        for (int i = 0; i < 256; i++) {
            ckey_pal_u32[i] = p->colors565[i];
        }
        ckey_pal_src = p;
        ckey_pal_version = p->version;
    }
    return ckey_pal_u32;
}
#endif
...
static void quad_infer_positions(SWQuad* quad) {
    float dx = quad->positions[3].x - quad->positions[0].x;
    float dy = quad->positions[3].y - quad->positions[0].y;
    quad->positions[1].x = quad->positions[0].x + dx;
    quad->positions[1].y = quad->positions[0].y;
    quad->positions[2].x = quad->positions[0].x;
    quad->positions[2].y = quad->positions[0].y + dy;
}

static void quad_infer_tex_coords(SWQuad* quad) {
    float ds = quad->tex_coords[3].s - quad->tex_coords[0].s;
    float dt = quad->tex_coords[3].t - quad->tex_coords[0].t;
    quad->tex_coords[1].s = quad->tex_coords[0].s + ds;
    quad->tex_coords[1].t = quad->tex_coords[0].t;
    quad->tex_coords[2].s = quad->tex_coords[0].s;
    quad->tex_coords[2].t = quad->tex_coords[0].t + dt;
}

static uint32_t float_to_sortable_u32(float f) {
    union {
        float f;
        uint32_t u;
    } u;
    u.f = f;
    if ((u.u & 0x80000000u) != 0) {
        return ~u.u;
    }
    return u.u | 0x80000000u;
}
...
static void insertion_sort_idx(uint16_t* idx, int lo, int hi, const uint64_t* keys) {
    for (int i = lo + 1; i <= hi; i++) {
        uint16_t key_idx = idx[i];
        uint64_t key_val = keys[key_idx];
        int j = i - 1;
        while (j >= lo && keys[idx[j]] > key_val) {
            idx[j + 1] = idx[j];
            j--;
        }
        idx[j + 1] = key_idx;
...
static uint32_t sample_texture_argb(const SWTexture* tex, const SWPalette* pal, float s, float t) {
    int tx = clamp_i((int)(s * (float)tex->width), 0, tex->width - 1);
    int ty = clamp_i((int)(t * (float)tex->height), 0, tex->height - 1);

    if (tex->palette_type == SW_PALETTE_8) {
        return pal->colors[((const uint8_t*)tex->pixels)[ty * tex->width + tx]];
    }
    if (tex->palette_type == SW_PALETTE_4) {
        const uint8_t byte = ((const uint8_t*)tex->pixels)[ty * (tex->width / 2) + (tx >> 1)];
        return pal->colors[(tx & 1) ? (byte >> 4) : (byte & 0x0F)];
    }
...
static void rasterize_textured_triangle(const SWQuad* q, RBCtx* ctx, int i0, int i1, int i2, const SWTexture* tex, const SWPalette* pal) {
    ...
    float s0 = q->tex_coords[i0].s, t0 = q->tex_coords[i0].t;
    float s1 = q->tex_coords[i1].s, t1 = q->tex_coords[i1].t;
    ...
    const int min_x = clamp_i(floor_to_int(min3f(x0, x1, x2)), 0, CANVAS_W);
    const int max_x = clamp_i(ceil_to_int(max3f(x0, x1, x2)), 0, CANVAS_W);
...
static void rasterize_textured(const SWQuad* q, RBCtx* ctx) {
    ...
    uint32_t v_fx = flip_y ? (uint32_t)((v_hi - flt_v_lead) * 65536.0f) : (uint32_t)((v_lo + flt_v_lead) * 65536.0f);
    ...
    for (int y = 0; y < dst_h; y++) {
        ...
        if (tex->palette_type == SW_PALETTE_8) {
            ...
            if (flip_x) {
                if (unscaled) {
                    int remaining = dst_w, src = src_x_start_rev;
                    SWCanvasPixel* d = drow;
                    while (remaining > 0) {
                ...
                int remaining = dst_w, src = src_x_start_fwd;
                SWCanvasPixel* d = drow;
                while (remaining > 0) {
...
// -----------------------------------------------------------------------------
// Texture & Palette VTable
// -----------------------------------------------------------------------------

static void sw_create_texture(unsigned int th) {
...
static void sw_destroy_texture(unsigned int texture_handle) {
    (void)texture_handle;
}

static void sw_unlock_texture(unsigned int th) {
    sw_create_texture(th);
}

static void sw_create_palette(unsigned int ph) {
...
static void sw_destroy_palette(unsigned int palette_handle) {
    (void)palette_handle;
}

static void sw_unlock_palette(unsigned int ph) {
    sw_create_palette(ph << 16);
}

static void sw_set_texture(unsigned int th) {
    ...
    latest_texture_spec.palette_index = palette_handle > 0 ? (int16_t)(palette_handle - 1) : -1;
}
...
static void sw_render_frame(void) {
    sort_quads_fast();
#if CRS_SW_DEDUP_FS_SOLIDS
    dedup_fs_solids();
#endif
    RBCtx main_ctx = { 0, CANVAS_H };
...
static void sw_bind_transient_rt(void* handle) {
    (void)handle;
}

static void sw_set_blend_mode(RendererBlendMode mode) {
    (void)mode;
}
...
const GameRendererVtable s_vtable_sw = {
    .Init = sw_init,
    .Shutdown = sw_shutdown,
    .BeginFrame = sw_begin_frame,
    .RenderFrame = sw_render_frame,
    .ExecutePass = sw_execute_pass,
    .EndFrame = sw_end_frame,

    .CreateTexture = sw_create_texture,
    .DestroyTexture = sw_destroy_texture,
    .UnlockTexture = sw_unlock_texture,
    .CreatePalette = sw_create_palette,
    .DestroyPalette = sw_destroy_palette,
    .UnlockPalette = sw_unlock_palette,
    .SetTexture = sw_set_texture,

    .CreateTransientRenderTarget = sw_create_transient_rt,
    .DestroyTransientRenderTarget = sw_destroy_transient_rt,
    .BindTransientRenderTarget = sw_bind_transient_rt,

    .SetBlendMode = sw_set_blend_mode,

    .DrawTexturedQuad = sw_draw_textured_quad,
    .DrawSolidQuad = sw_draw_solid_quad,
    .DrawSprite = sw_draw_sprite,
    .DrawSprite2 = sw_draw_sprite2,
    .FlushSprite2Batch = sw_flush_sprite2_batch,

    .GetCachedGLTexture = sw_get_cached_gl_texture,
    .DumpTextures = sw_dump_textures,

    .DrawOverlayQuad = sw_draw_overlay_quad,
    .DrawOverlayQuadEx = sw_draw_overlay_quad_ex,
};
