...
static const ControllerImage_MouseIconName mouse_icon_names[] = {
    { "none", CONTROLLERIMAGE_MOUSE_NONE },     { "left", CONTROLLERIMAGE_MOUSE_LEFT },
    { "middle", CONTROLLERIMAGE_MOUSE_MIDDLE }, { "right", CONTROLLERIMAGE_MOUSE_RIGHT },
    { "x1", CONTROLLERIMAGE_MOUSE_X1 },         { "x2", CONTROLLERIMAGE_MOUSE_X2 },
    { "move", CONTROLLERIMAGE_MOUSE_MOVE },     { "scroll", CONTROLLERIMAGE_MOUSE_SCROLL },
};
...
typedef struct ControllerImage_KeyboardDevice {
    // any of these might be NULL!
    NSVGimage* keys[SDL_SCANCODE_COUNT][CONTROLLERIMAGE_MAX_KEYBOARD_VARIANTS];
    char* keys_svg[SDL_SCANCODE_COUNT][CONTROLLERIMAGE_MAX_KEYBOARD_VARIANTS];
    NSVGrasterizer* rasterizer;
} ControllerImage_KeyboardDevice;

typedef struct ControllerImage_GamepadDevice {
    // any of these might be NULL!
    NSVGimage* axes[SDL_GAMEPAD_AXIS_COUNT][CONTROLLERIMAGE_MAX_GAMEPAD_VARIANTS];
    NSVGimage* buttons[SDL_GAMEPAD_BUTTON_COUNT][CONTROLLERIMAGE_MAX_GAMEPAD_VARIANTS];
    const char* device_type;
    char* axes_svg[SDL_GAMEPAD_AXIS_COUNT][CONTROLLERIMAGE_MAX_GAMEPAD_VARIANTS];
    char* buttons_svg[SDL_GAMEPAD_BUTTON_COUNT][CONTROLLERIMAGE_MAX_GAMEPAD_VARIANTS];
    NSVGrasterizer* rasterizer;
...
int ControllerImage_LinkedVersion(void) {
    return CONTROLLERIMAGE_MAJOR_VERSION * 1000000 + CONTROLLERIMAGE_MINOR_VERSION * 1000 + CONTROLLERIMAGE_PATCHLEVEL;
}
...
static void SDLCALL CleanupDeviceInfo(void* userdata, void* value) {
    ...
    if (info->refcount == 1) {
        SDL_free(value); // this is the ControllerImage_GamepadDeviceInfo and also its items in the same buffer. Strings
                         // live elsewhere and are reused.
    } else {
        info->refcount--;
    }
}
...
int ControllerImage_AddData(const void* buf, size_t buflen) {
    ...
    for (Uint16 i = 0; i < num_devices; i++) {
        ...
        ControllerImage_GamepadDeviceInfo* info = (ControllerImage_GamepadDeviceInfo*)SDL_calloc(
            1, sizeof(ControllerImage_GamepadDeviceInfo) + (sizeof(ControllerImage_Item) * num_items));
        if (!info) {
            goto failed;
        }
        ...
        if (!SDL_SetPointerPropertyWithCleanup(DeviceGuidMap, strings[devid], info, CleanupDeviceInfo, NULL)) {
            SDL_free(info);
            goto failed;
        }
        ...
        for (Uint16 j = 0; j < num_guids; j++) {
            ...
            // !!! FIXME: this does not work with overriding controllers with a second data set (so you load `standard`
            // and then load `kenney` on top),
            // !!! FIXME:  so looking up a device by GUID will find the original images, not the overridden ones.
            // If this fails for some reason, go on without this guid. Do NOT SDL_free(info), it's already in the
            // database, refcounted.
            if (SDL_SetPointerPropertyWithCleanup(DeviceGuidMap, guidstr, info, CleanupDeviceInfo, NULL)) {
                info->refcount++;
            }
        }
    }
...
ControllerImage_GamepadDevice* ControllerImage_CreateGamepadDeviceByIdString(const char* str) {
    return CreateGamepadDeviceFromInfo(
        (ControllerImage_GamepadDeviceInfo*)SDL_GetPointerProperty(DeviceGuidMap, str, NULL));
}
...
ControllerImage_GamepadDevice* ControllerImage_CreateGamepadDeviceByInstance(SDL_JoystickID jsid) {
    ...
    ControllerImage_GamepadDeviceInfo* info =
        (ControllerImage_GamepadDeviceInfo*)SDL_GetPointerProperty(DeviceGuidMap, guidstr, NULL);

    if (!info) {
        guidstr[4] = guidstr[5] = guidstr[6] = guidstr[7] = '0'; // clear out the CRC, see if it matches...
        info = (ControllerImage_GamepadDeviceInfo*)SDL_GetPointerProperty(DeviceGuidMap, guidstr, NULL);
    }
    ...
    if (!info) {
        ...
        if (typestr) {
            info = (ControllerImage_GamepadDeviceInfo*)SDL_GetPointerProperty(DeviceGuidMap, typestr, NULL);
        }
    }

    if (!info) {
        info = (ControllerImage_GamepadDeviceInfo*)SDL_GetPointerProperty(
            DeviceGuidMap, "xbox360", NULL); // if all else fails, this is probably most likely to match...
    }
...
ControllerImage_MouseDevice* ControllerImage_CreateMouseDevice() {
    ...
    ControllerImage_GamepadDeviceInfo* info =
        (ControllerImage_GamepadDeviceInfo*)SDL_GetPointerProperty(DeviceGuidMap, "mouse", NULL);
...
ControllerImage_KeyboardDevice* ControllerImage_CreateKeyboardDevice() {
    ...
    ControllerImage_GamepadDeviceInfo* info =
        (ControllerImage_GamepadDeviceInfo*)SDL_GetPointerProperty(DeviceGuidMap, "keyboard", NULL);
...
SDL_Surface* ControllerImage_CreateSurfaceForAxis(ControllerImage_GamepadDevice* device, SDL_GamepadAxis axis, int size,
                                                  int variantID) {
...
SDL_Surface* ControllerImage_CreateSurfaceForButton(ControllerImage_GamepadDevice* device, SDL_GamepadButton button,
                                                    int size, int variantID) {
...
SDL_Surface* ControllerImage_CreateSurfaceForScancode(ControllerImage_KeyboardDevice* device, SDL_Scancode scancode,
                                                      int size, int variantID) {
    const int iscancode = (int)scancode;
    if ((iscancode < SDL_SCANCODE_UNKNOWN) || (iscancode >= SDL_SCANCODE_COUNT)) {
...
SDL_Surface* ControllerImage_CreateSurfaceForMouseIcon(ControllerImage_MouseDevice* device, int iconID, int size,
                                                       int variantID) {
...
const char* ControllerImage_GetSVGForButton(ControllerImage_GamepadDevice* device, SDL_GamepadButton button,
                                            int variantID) {
    const int ibutton = (int)button;
    if ((ibutton < 0) || (ibutton >= SDL_GAMEPAD_BUTTON_COUNT)) {
        SDL_InvalidParamError("button");
...
const char* ControllerImage_GetSVGForScancode(ControllerImage_KeyboardDevice* device, SDL_Scancode scancode,
                                              int variantID) {
    const int iscancode = (int)scancode;
    if ((iscancode < SDL_SCANCODE_UNKNOWN) || (iscancode >= SDL_SCANCODE_COUNT)) {
        SDL_InvalidParamError("scancode");
...
bool ControllerImage_HasScancodeSVG(ControllerImage_KeyboardDevice* device, SDL_Scancode scancode, int variantID) {
    const int iscancode = (int)scancode;
    if ((iscancode < SDL_SCANCODE_UNKNOWN) || (iscancode >= SDL_SCANCODE_COUNT)) {
        SDL_InvalidParamError("scancode");
        return false;
    ...
    if (!device->keys_svg[iscancode][variantID])
        return false;
    return true;
}

bool ControllerImage_HasMouseIconSVG(ControllerImage_MouseDevice* device, int iconID, int variantID) {
    if ((iconID < CONTROLLERIMAGE_MOUSE_NONE) || (iconID >= CONTROLLERIMAGE_MOUSE_MAX)) {
        SDL_InvalidParamError("mouse icon");
        return false;
    }
    ...
    if (!device->icons_svg[iconID][variantID])
        return false;
    return true;
}
