...
int ControllerImage_LinkedVersion(void) { return CONTROLLERIMAGE_MAJOR_VERSION * 1000000 + CONTROLLERIMAGE_MINOR_VERSION * 1000 + CONTROLLERIMAGE_PATCHLEVEL; }
...
static void CollectGamepadImages(ControllerImage_GamepadDeviceInfo *info, ControllerImage_GamepadDevice *device)
{
    if (!info) {
        ...
        CollectGamepadImages((ControllerImage_GamepadDeviceInfo *) SDL_GetPointerProperty(DeviceGuidMap, info->inherits, NULL), device);
    }
...
static void CollectKeyboardImages(ControllerImage_GamepadDeviceInfo *info, ControllerImage_KeyboardDevice *device)
{
    if (!info) {
        ...
        CollectKeyboardImages((ControllerImage_GamepadDeviceInfo *) SDL_GetPointerProperty(DeviceGuidMap, info->inherits, NULL), device);
    }
...
static void CollectMouseImages(ControllerImage_GamepadDeviceInfo *info, ControllerImage_MouseDevice *device)
{
    if (!info) {
        ...
        CollectMouseImages((ControllerImage_GamepadDeviceInfo *) SDL_GetPointerProperty(DeviceGuidMap, info->inherits, NULL), device);
    }
...
{
    return CreateGamepadDeviceFromInfo((ControllerImage_GamepadDeviceInfo *) SDL_GetPointerProperty(DeviceGuidMap, str, NULL));
}
...
ControllerImage_GamepadDevice *ControllerImage_CreateGamepadDeviceByInstance(SDL_JoystickID jsid)
{
    ...
    ControllerImage_GamepadDeviceInfo *info = (ControllerImage_GamepadDeviceInfo *) SDL_GetPointerProperty(DeviceGuidMap, guidstr, NULL);
    ...
    if (!info) {
        info = (ControllerImage_GamepadDeviceInfo *) SDL_GetPointerProperty(DeviceGuidMap, "xbox360", NULL);  // if all else fails, this is probably most likely to match...
    }
...
ControllerImage_MouseDevice *ControllerImage_CreateMouseDevice()
{
    ...
    ControllerImage_GamepadDeviceInfo *info = (ControllerImage_GamepadDeviceInfo *) SDL_GetPointerProperty(DeviceGuidMap, "mouse", NULL);
...
ControllerImage_KeyboardDevice *ControllerImage_CreateKeyboardDevice()
{
    ...
    ControllerImage_GamepadDeviceInfo *info = (ControllerImage_GamepadDeviceInfo *) SDL_GetPointerProperty(DeviceGuidMap, "keyboard", NULL);
...
bool ControllerImage_HasScancodeSVG(ControllerImage_KeyboardDevice *device, SDL_Scancode scancode, int variantID)
{
    ...
    if (!device->keys_svg[iscancode][variantID]) return false;
    return true;
}
...
bool ControllerImage_HasMouseIconSVG(ControllerImage_MouseDevice *device, int iconID, int variantID)
{
    ...
    if (!device->icons_svg[iconID][variantID]) return false;
    return true;
}