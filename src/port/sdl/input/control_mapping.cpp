...
static void try_auto_assign_defaults() {
    ...
    // Try to auto-assign P2
    if (!p2Device) {
        for (auto it = availableDevices.begin(); it != availableDevices.end(); ++it) {
            if (!SDLPad_IsKeyboard(it->id)) {
                p2Device = std::make_unique<Device>(*it);
                p2MappingState = MappingState::Idle;
                player_mappings[2].clear();
                for (const auto& def : default_gamepad_mappings) {
                    player_mappings[2].push_back({ def.action, def.input_id });
                }
                SDL_Log("Auto-assigned gamepad '%s' to P2 with default mappings", p2Device->name.c_str());
                availableDevices.erase(it);
                mapped_anything = true;
                break;
            }
...
const char* ControlMapping_GetPlayerMappingIconURI(int player_num, int index) {
    ...
    if (is_keyboard_input(id)) {
        int scancode = (int)id - INPUT_ID_KEY_BASE;
        SDL_snprintf(s_uri_buf, sizeof(s_uri_buf), "ctrlimg:scancode:%d", scancode);
        return s_uri_buf;
    }
    ...
    if (input_to_gamepad_btn(id, &out_btn)) {
        SDL_snprintf(s_uri_buf, sizeof(s_uri_buf), "ctrlimg:device:%d;button:%d", dev->id, (int)out_btn);
        return s_uri_buf;
    }

    SDL_GamepadAxis out_axis;
    if (input_to_gamepad_axis(id, &out_axis)) {
        SDL_snprintf(s_uri_buf, sizeof(s_uri_buf), "ctrlimg:device:%d;axis:%d", dev->id, (int)out_axis);
        return s_uri_buf;
    }
