...
extern "C" void rmlui_win_screen_init(void) {
    ...
    ctor.BindFunc("winner_name", [](Rml::Variant& v){
        v = Rml::String(char_name(My_char[Winner_id]));
    });
    ctor.BindFunc("winner_score", [](Rml::Variant& v){
        v = (int)WGJ_Score;
    });
    ctor.BindFunc("winner_wins", [](Rml::Variant& v){
        v = (int)WGJ_Win;
    });
    ctor.BindFunc("is_loser", [](Rml::Variant& v){
        v = false; // Updated per-frame via dirty check
    });
    ctor.BindFunc("is_versus", [](Rml::Variant& v){
        v = (bool)(Mode_Type == MODE_VERSUS);
    });
...
extern "C" void rmlui_win_screen_show(void) {
    ...
    if (s_model_handle) s_model_handle.DirtyVariable("is_loser");
...
extern "C" void rmlui_win_screen_shutdown(void) {
    if (s_model_registered) {
        rmlui_wrapper_hide_document("win");
        Rml::Context* ctx = static_cast<Rml::Context*>(rmlui_wrapper_get_context());
        if (ctx) ctx->RemoveDataModel("win_screen");
