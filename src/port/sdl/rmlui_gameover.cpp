...
extern "C" void rmlui_gameover_init(void) {
    ...
    ctor.BindFunc("gameover_score", [](Rml::Variant& v){
        v = (int)Score[Player_id][Play_Type];
    });
    ctor.BindFunc("gameover_char", [](Rml::Variant& v){
        v = Rml::String(char_name(My_char[Player_id]));
    });
    ctor.BindFunc("gameover_rounds_won", [](Rml::Variant& v){
        v = (int)Win_Record[Player_id];
    });
...
extern "C" void rmlui_gameover_shutdown(void) {
    if (s_model_registered) {
        rmlui_wrapper_hide_document("gameover");
        Rml::Context* ctx = static_cast<Rml::Context*>(rmlui_wrapper_get_context());
        if (ctx) ctx->RemoveDataModel("gameover_screen");
