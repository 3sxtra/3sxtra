...
static inline int cps3_r(u16 c) {
    return (c >> 0) & 0x1F;
}
static inline int cps3_g(u16 c) {
    return (c >> 5) & 0x1F;
}
static inline int cps3_b(u16 c) {
    return (c >> 10) & 0x1F;
}
...
static inline int to8bit(int v5) {
    return (v5 << 3) | (v5 >> 2);
}
...
static const char* s_color_labels[] = { "LP",       "MP",      "HP",      "LK",      "MK",      "HK",
                                        "LP+MK+HP", "Extra-1", "Extra-2", "Extra-3", "Extra-4", "Extra-5",
...
static RemixEffectState s_remix_effects[] = { { "chk_noir", "Noir", FX_MOOD, false, 50 },
...
extern "C" void rmlui_palmod_menu_flush_config(void) {
    flush_config();
}
...
static void do_init(void) {
    ...
    ctor.BindFunc(
        ...
        [](const Rml::Variant& v) {
            int val = v.Get<int>();
            if (val < STAGE_ROW_MIN)
                val = STAGE_ROW_MIN;
            if (val > STAGE_ROW_MAX)
                val = STAGE_ROW_MAX;
            s_stage_row = val;
        });
    ...
    ctor.BindFunc(
        ...
        [](const Rml::Variant& v) {
            ...
            if (s_picker_r > 31)
                s_picker_r = 31;
            if (s_picker_active)
                write_single_color(s_picker_row, s_picker_index, cps3_pack(s_picker_r, s_picker_g, s_picker_b));
        });
    ...
    ctor.BindFunc("sel_preview",
                  [](Rml::Variant& v) { v = color_to_css(cps3_pack(s_picker_r, s_picker_g, s_picker_b)); });

    ctor.BindFunc("sel_original", [](Rml::Variant& v) { v = color_to_css(s_picker_original); });

    /* ─── Event callbacks ─── */

    ctor.BindEventCallback("pick_p1_color", [](Rml::DataModelHandle, Rml::Event& ev, const Rml::VariantList& args) {
        if (args.empty())
            return;
    ...
    ctor.BindEventCallback("pick_p2_color", [](Rml::DataModelHandle, Rml::Event& ev, const Rml::VariantList& args) {
        if (args.empty())
            return;
        int idx = args[0].Get<int>();
        if (idx < 0 || idx >= 64)
    ...
    ctor.BindEventCallback("pick_stage_color", [](Rml::DataModelHandle, Rml::Event& ev, const Rml::VariantList& args) {
        if (args.empty())
            return;
        int idx = args[0].Get<int>();
        if (idx < 0 || idx >= 64)
    ...
    ctor.BindEventCallback("reset_color",
                           [](Rml::DataModelHandle, Rml::Event&, const Rml::VariantList&) { reset_picker_color(); });

    ctor.BindEventCallback("close_picker",
                           [](Rml::DataModelHandle, Rml::Event&, const Rml::VariantList&) { close_picker(); });

    ctor.BindEventCallback("set_mode_chars", [](Rml::DataModelHandle, Rml::Event&, const Rml::VariantList&) {
        s_edit_mode = 0;
        s_list_dirty = true;
    });

    ctor.BindEventCallback("set_mode_stage", [](Rml::DataModelHandle, Rml::Event&, const Rml::VariantList&) {
        s_edit_mode = 1;
        s_list_dirty = true;
    });

    ctor.BindEventCallback("set_mode_remix", [](Rml::DataModelHandle, Rml::Event&, const Rml::VariantList&) {
        s_edit_mode = 2;
        s_list_dirty = true;
    ...
    ctor.BindFunc(
        ...
        [](const Rml::Variant& v) {
            s_remix_target_stage = v.Get<bool>();
            trigger_remix();
        });
    ...
    ctor.BindFunc(
        ...
        [](const Rml::Variant& v) {
            s_remix_auto_preview = v.Get<bool>();
            trigger_remix();
        });
    ...
    for (int i = 0; i < REMIX_EFFECT_COUNT; i++) {
        ...
        ctor.BindFunc(
            ...
            [i](const Rml::Variant& v) {
                ...
                SDL_Log("[PalMod Remix] chk_%s setter: %d -> %d",
                        s_remix_effects[i].id,
                        s_remix_effects[i].checked,
                        newval);
    ...
    ctor.BindEventCallback("toggle_fx", [](Rml::DataModelHandle h, Rml::Event& ev, const Rml::VariantList& args) {
        if (args.empty())
            return;
        int i = args[0].Get<int>();
        if (i < 0 || i >= REMIX_EFFECT_COUNT)
            return;
    ...
    ctor.BindEventCallback("fx_inc", [](Rml::DataModelHandle h, Rml::Event& ev, const Rml::VariantList& args) {
        if (args.empty())
            return;
        int i = args[0].Get<int>();
        if (i < 0 || i >= REMIX_EFFECT_COUNT)
    ...
    ctor.BindEventCallback("fx_dec", [](Rml::DataModelHandle h, Rml::Event& ev, const Rml::VariantList& args) {
        if (args.empty())
            return;
        int i = args[0].Get<int>();
        if (i < 0 || i >= REMIX_EFFECT_COUNT)
    ...
    ctor.BindEventCallback("remix_reset", [](Rml::DataModelHandle, Rml::Event&, const Rml::VariantList&) {
        for (int i = 0; i < REMIX_EFFECT_COUNT; i++) {
    ...
    ctor.BindEventCallback("remix_apply", [](Rml::DataModelHandle, Rml::Event&, const Rml::VariantList&) {
        apply_remix_preview();
        if (s_remix_target_p1) {
            for (int r = 0; r < 16; r++)
    ...
    ctor.BindFunc("apply_flash", [](Rml::Variant& v) { v = (s_apply_flash_timer > 0); });
    ...
    ctor.BindFunc(
        ...
        [](const Rml::Variant& v) {
            ...
            if (len >= sizeof(s_save_name))
                len = sizeof(s_save_name) - 1;
            memcpy(s_save_name, s_save_name_str.c_str(), len);
            s_save_name[len] = '\0';
        });

    ctor.BindFunc("saved_list", [](Rml::Variant& v) { v = s_palette_list_str; });
    ...
    ctor.BindFunc(
        ...
        [](const Rml::Variant& v) {
            ...
            if (s_selected_palette < 0)
                s_selected_palette = 0;
    ...
    ctor.BindEventCallback("do_save",
                           [](Rml::DataModelHandle, Rml::Event&, const Rml::VariantList&) { do_save_current_row(); });

    ctor.BindEventCallback("do_load",
                           [](Rml::DataModelHandle, Rml::Event&, const Rml::VariantList&) { do_load_selected(); });

    ctor.BindEventCallback("do_delete",
                           [](Rml::DataModelHandle, Rml::Event&, const Rml::VariantList&) { do_delete_selected(); });

    ctor.BindEventCallback("prev_palette", [](Rml::DataModelHandle, Rml::Event&, const Rml::VariantList&) {
        if (s_selected_palette > 0)
            s_selected_palette--;
    });

    ctor.BindEventCallback("next_palette", [](Rml::DataModelHandle, Rml::Event&, const Rml::VariantList&) {
        if (s_selected_palette < s_palette_count - 1)
            s_selected_palette++;
    });
    ...
    for (int i = 0; i < 64; i++) {
        ...
        ctor.BindFunc(Rml::String(name), [i](Rml::Variant& v) {
    ...
    for (int i = 0; i < 64; i++) {
        ...
        ctor.BindFunc(Rml::String(name), [i](Rml::Variant& v) { v = color_to_css(ColorRAM[16][i]); });
    }
    ...
    for (int row_ofs = 0; row_ofs <= 3; row_ofs++) {
        for (int i = 0; i < 64; i++) {
            ...
            ctor.BindFunc(Rml::String(name), [row_ofs, i](Rml::Variant& v) {
    ...
    for (int row_ofs = 0; row_ofs <= 3; row_ofs++) {
        ...
        ctor.BindFunc(Rml::String(name), [row_ofs](Rml::Variant& v) {
            char buf[16];
    ...
    SDL_LogDebug(SDL_LOG_CATEGORY_APPLICATION, "[RmlUi Palmod] Data model registered (%d bindings)", 128 + 20);
}

extern "C" void rmlui_palmod_menu_init(void) {
    do_init();
}
