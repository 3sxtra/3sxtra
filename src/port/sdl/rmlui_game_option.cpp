...
static const OptRow s_rows[10] = {
    ...
    {"ROUNDS (1P)",    rounds_str},
    {"ROUNDS (VS)",    rounds_str},
    {"DAMAGE LEVEL",   damage_str},
    {"GUARD JUDGMENT", guard_str},
    {"ANALOG STICK",   enabled_str},
    {"HANDICAP (VS)",  onoff_str},
    {"PLAYER1 (VS)",   human_com_str},
...
extern "C" void rmlui_game_option_show(void) {
    rmlui_wrapper_show_document("game_option");
    if (s_model_handle) {
        s_model_handle.DirtyVariable("game_opt_cursor");
        for (int i = 0; i < 10; i++) {
            char name[32];
...
extern "C" void rmlui_game_option_hide(void) {
    rmlui_wrapper_hide_document("game_option");
}
...
extern "C" void rmlui_game_option_shutdown(void) {
    if (s_model_registered) {
        rmlui_wrapper_hide_document("game_option");
        Rml::Context* ctx = static_cast<Rml::Context*>(rmlui_wrapper_get_context());
        if (ctx) ctx->RemoveDataModel("game_option");
