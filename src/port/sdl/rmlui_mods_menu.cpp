...
extern "C" void rmlui_mods_menu_init(void) {
    ...
    // --- Debug HUD ---
    constructor.BindFunc("debug_hud",
...
extern "C" void rmlui_mods_menu_update(void) {
    ...
    #define DIRTY_BOOL(name, expr) do { \
        bool _cur = (expr); \
        if (_cur != s_cache.name) { s_cache.name = _cur; s_model_handle.DirtyVariable(#name); } \
    } while(0)

    #define DIRTY_INT(name, expr) do { \
        int _cur = (expr); \
        if (_cur != s_cache.name) { s_cache.name = _cur; s_model_handle.DirtyVariable(#name); } \
    } while(0)
