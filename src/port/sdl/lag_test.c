...
void GpioLagTest_Init(void) {
    ...
    s_gpio_ready = false;
    s_enabled = true;
    SDL_Log("[GpioLagTest] Initialized (Gamepad mode only)");
#endif
}
...
void GpioLagTest_OnInputPoll(void) {
    ...
    /* If GPIO is available, read from it */
#ifdef __linux__
    if (s_gpio_ready && s_line_fd >= 0) {
        struct gpio_v2_line_values vals;
        memset(&vals, 0, sizeof(vals));
        vals.mask = 1;
...
#endif
    {
        /* Fallback: use player 1 gamepad attack buttons (LP, MP, HP, LK, MK, HK) */
        /* Flags: 0x10, 0x20, 0x40, 0x100, 0x200, 0x400 -> total 0x770 */
        s_button_held = (io_w.sw[0] & 0x770) != 0;
    }
...
void GpioLagTest_Toggle(void) {
    s_enabled = !s_enabled;
    if (s_enabled && !s_gpio_ready) {
        SDL_Log("[GpioLagTest] Enabled in Gamepad/Software mode (GPIO not initialized)");
    } else {
