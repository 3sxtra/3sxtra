...
int ADX_InitContext(ADXContext* ctx, const u8* header, size_t header_size) {
    ...
    // Calculate coefficients based on standard CRI ADX algorithm (cutoff from header or default to 500Hz)
    double cutoff = 500.0;
    if (header_size >= 18) {
        u16 cutoff_freq = read_u16be(header + 0x10);
        if (cutoff_freq > 0) {
            cutoff = (double)cutoff_freq;
        }
    }
