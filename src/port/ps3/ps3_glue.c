/**
 * @file ps3_glue.c
 * @brief PS3 port glue layer — bridges SDL API calls to PS3 native implementations.
 *
...
void TextureUtil_DrawQuadEx(void* texture_id, float x, float y, float w, float h, float z, int flip_x, int flip_y) {
    ...
    v[0].u = flip_x ? 1.0f : 0.0f;
    v[0].v = flip_y ? 1.0f : 0.0f;
    v[1].x = x + w;
    v[1].y = y;
    v[1].z = z;
    v[1].color = 0xFFFFFFFF;
    v[1].u = flip_x ? 0.0f : 1.0f;
    v[1].v = flip_y ? 1.0f : 0.0f;
    v[2].x = x;
    v[2].y = y + h;
    v[2].z = z;
    v[2].color = 0xFFFFFFFF;
    v[2].u = flip_x ? 1.0f : 0.0f;
    v[2].v = flip_y ? 0.0f : 1.0f;
    v[3].x = x + w;
    v[3].y = y + h;
    v[3].z = z;
    v[3].color = 0xFFFFFFFF;
    v[3].u = flip_x ? 0.0f : 1.0f;
    v[3].v = flip_y ? 0.0f : 1.0f;
    Renderer_DrawTexturedQuadVtx(v, 4);
}

// Structural / Engine / Window stubs for PS3 port
int Renderer_LZ77Enqueue(const unsigned char* a, unsigned int b, unsigned int c, int d, int e, unsigned int f,
                         unsigned int g) {
    return 0;
}
void* LoadFullSpriteOverride(unsigned char a, unsigned short b, unsigned char c) {
    return 0;
}
...
Uint64 SDLApp_GetTargetFrameTimeNS(void) {
    return PS3App_GetTargetFrameTimeNS();
}
SDL_Window* SDLApp_GetWindow(void) {
    return 0;
}
int SDLApp_Init() {
    return PS3App_FullInit();
}
bool SDLApp_IsFrameRateUncapped(void) {
    return PS3App_IsFrameRateUncapped();
}
bool SDLApp_IsVSyncEnabled(void) {
    return PS3App_IsVSyncEnabled();
}
bool SDLApp_PollEvents() {
    return PS3App_PollEvents();
}
...
bool SDLPad_GetJoystickAxis(int a, int b, int sign) {
    if (!SDLPad_IsGamepadConnected(a))
        return false;
    ...
    switch (b) {
    case 0:
        val = state.left_stick_x;
        break;
    case 1:
        val = state.left_stick_y;
        break;
    case 2:
        val = state.right_stick_x;
        break;
    case 3:
        val = state.right_stick_y;
        break;
    case 4:
        val = state.left_trigger;
        break;
    case 5:
        val = state.right_trigger;
        break;
    }
    if (sign == 0)
        return val > 16000;
...
bool SDLPad_IsJoystick(int a) {
    return SDLPad_IsGamepadConnected(a);
}
bool SDLPad_IsKeyboard(int a) {
    return false;
}
...
extern void SPU_RunTimerTick(void);
extern void* soundLock;

void SPU_TickAudio(int16_t* outbuf, uint32_t samples_per_channel) {
    // H-05 Audit Fix: Guard against NULL soundLock to prevent race during init
    if (!soundLock) {
        memset(outbuf, 0, samples_per_channel * 2 * sizeof(int16_t));
        return;
    }
...
bool is_joystick_input(InputID a) {
    return (a >= INPUT_ID_JOY_BASE);
}
bool is_keyboard_input(InputID a) {
    return (a >= INPUT_ID_KEY_BASE && a < INPUT_ID_JOY_BASE);
}

int g_resolution_scale = 1;

// UI/RmlUi Linker Stubs
int rmlui_casual_lobby_is_visible(void) {
    return 0;
}
int rmlui_tournament_lobby_is_visible(void) {
    return 0;
}
