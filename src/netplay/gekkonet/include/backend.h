...
namespace Gekko {
	...
	public:
        ...
        void SendStateChunks(NetAddress& addr, Frame frame, u8* state_data, u32 state_size, GekkoNetAdapter* host);

        void SendStateChunkRequest(NetAddress* addr, GekkoNetAdapter* host);

        std::vector<NetAddress> GetAndClearStateRequests();

        bool IsStateFullyAssembled() const;

        std::vector<u8> GetAssembledState() const;

        Frame GetAssembledStateFrame() const;

        void ClearAssembledState();
	...
	private:
        ...
        void OnStateChunkRequest(NetAddress& addr, NetPacket& pkt);

        void OnStateChunkResponse(NetAddress& addr, NetPacket& pkt);

        void OnStateChunkAck(NetAddress& addr, NetPacket& pkt);
	...
	private:
        ...
        std::vector<NetAddress> _state_chunk_requests;

        struct IncomingStateChunk {
            Frame frame = -1;
            u32 total_size = 0;
            u16 total_chunks = 0;
