...
void Gekko::MessageSystem::ParsePacket(NetAddress& addr, NetPacket& pkt, u32 packet_size)
{
    ...
    if (pkt.header.magic != _session_magic) {
        ...
        case NetworkHealth:
            ...
            return;
        case StateChunkRequest:
            OnStateChunkRequest(addr, pkt);
            return;
        case StateChunkResponse:
            OnStateChunkResponse(addr, pkt);
            return;
        case StateChunkAck:
            OnStateChunkAck(addr, pkt);
            return;
        default:
...
void Gekko::MessageSystem::OnStateChunkRequest(NetAddress& addr, NetPacket& pkt)
{
    NetAddress copy;
    copy.Copy(&addr);
    _state_chunk_requests.push_back(std::move(copy));
}

void Gekko::MessageSystem::OnStateChunkResponse(NetAddress& addr, NetPacket& pkt)
{
    auto body = (StateChunkResponseMsg*)pkt.body.get();

    // Ignore older or different states
    if (_incoming_state.frame != body->frame) {
        _incoming_state.frame = body->frame;
        _incoming_state.total_size = body->total_size;
        _incoming_state.total_chunks = body->total_chunks;
        _incoming_state.assembled_data.assign(body->total_size, 0);
        _incoming_state.ack_bitmask = 0;
    }

    if (!(_incoming_state.ack_bitmask & (1 << body->chunk_index))) {
        u32 offset = body->chunk_index * 1024; // chunk_payload_size
        u32 chunk_len = (u32)body->chunk_data.size();
        
        if (offset + chunk_len <= _incoming_state.total_size) {
            std::copy(body->chunk_data.begin(), body->chunk_data.end(), _incoming_state.assembled_data.begin() + offset);
            _incoming_state.ack_bitmask |= (1 << body->chunk_index);
        }
    }
}

void Gekko::MessageSystem::OnStateChunkAck(NetAddress& addr, NetPacket& pkt)
{
    // Host could process missing chunks here for efficiency, but we are spamming so we can ignore it for now.
}

std::vector<Gekko::NetAddress> Gekko::MessageSystem::GetAndClearStateRequests()
{
    auto reqs = std::move(_state_chunk_requests);
    _state_chunk_requests.clear();
    return reqs;
}

bool Gekko::MessageSystem::IsStateFullyAssembled() const
{
    if (_incoming_state.total_chunks == 0) return false;
    u32 required_mask = (1 << _incoming_state.total_chunks) - 1;
    return (_incoming_state.ack_bitmask == required_mask);
}

std::vector<u8> Gekko::MessageSystem::GetAssembledState() const
{
    return _incoming_state.assembled_data;
}

Frame Gekko::MessageSystem::GetAssembledStateFrame() const
{
    return _incoming_state.frame;
}

void Gekko::MessageSystem::ClearAssembledState()
{
    _incoming_state.frame = -1;
    _incoming_state.total_size = 0;
    _incoming_state.total_chunks = 0;
    _incoming_state.assembled_data.clear();
    _incoming_state.ack_bitmask = 0;
}
...
void Gekko::MessageSystem::SendStateChunks(NetAddress& addr, Frame frame, u8* state_data, u32 state_size, GekkoNetAdapter* host)
{
    ...
    for (u16 i = 0; i < total_chunks; i++) {
        _pending_output.push(std::make_unique<NetData>());
        auto& message = _pending_output.back();

        u32 offset = i * chunk_payload_size;
        u32 this_chunk_size = std::min(chunk_payload_size, state_size - offset);

        auto body = std::make_unique<StateChunkResponseMsg>();
        body->frame = frame;
        body->total_size = state_size;
        body->chunk_index = i;
...
void Gekko::MessageSystem::SendStateChunkRequest(NetAddress* addr, GekkoNetAdapter* host)
{
    if (!addr) return;

    u64 now = TimeSinceEpoch();
    static u64 last_sent = 0;
    if (now - last_sent < 1000) return; // throttle requests to once per second
    last_sent = now;
    ...
    for (auto& remote : remotes) {
        if (remote->address.Equals(*addr)) {
            magic = remote->session_magic;
            break;
        }
    }
    ...
    message->pkt.header.magic = magic;
    message->pkt.body = std::make_unique<StateChunkRequestMsg>();

    SendDataTo(message.get(), host);
    _pending_output.pop();
}
