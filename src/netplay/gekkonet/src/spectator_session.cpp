...
void Gekko::SpectatorSession::SetStateCallbacks(GekkoStateCallbacks* callbacks)
{
    if (callbacks) {
...
{
    // Check for fully assembled state chunk
    if (_msg.IsStateFullyAssembled()) {
        auto state_data = _msg.GetAssembledState();
        Frame state_frame = _msg.GetAssembledStateFrame();
        
        if (_state_callbacks.inject_state) {
            _state_callbacks.inject_state(state_data.data(), (u32)state_data.size());
            _sync.SetCurrentFrame(state_frame);
            _last_saved_frame = state_frame - 1; // prevent immediate rollback bugs
        }
        
        _msg.ClearAssembledState();
    }
    ...
    for (auto& remote : _msg.remotes) {
        ...
        if (last_added_host_input > current + 120) { // 120 frames = 2 seconds behind
            _msg.SendStateChunkRequest(&remote->address, _host);
        }
        ...
        for (u32 i = 0; i < handles.size(); i++) {
            ...
            for (int j = last_recv; j <= last_added; j++) {
                if (j >= min_frame) {
                    int current_idx = j - min_frame;
                    u8* input = input_q[current_idx].get();
                    _sync.AddRemoteInput(handle, input, j);
                    _msg.SendInputAck(handle, j, 0);
                }
            }
        }
    }
