...
void Gekko::GameSession::SetStateCallbacks(GekkoStateCallbacks* callbacks)
{
    if (callbacks) {
...
void Gekko::GameSession::Poll()
{
    ...
    // handle any state chunk requests (from late-joining spectators)
    ...
    if (!state_reqs.empty() && _state_callbacks.get_state) {
        ...
        if (actual_size > 0 && actual_size <= state_buffer.size()) {
            for (auto& req_addr : state_reqs) {
                _msg.SendStateChunks(req_addr, _sync.GetCurrentFrame(), state_buffer.data(), actual_size, _host);
            }
        }
    }
