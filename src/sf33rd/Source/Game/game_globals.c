/**
 * @file game_globals.c
 * @brief Central global variable definitions for the game module.
 *
 * Defines the ~300 gameplay globals that drive the entire game: player state,
 * scores, round/stage control, mode flags, AI work buffers, combo system data,
 * character select state, and more. These are referenced across virtually every
 * subsystem in src/sf33rd/Source/Game/.
 *
 * Many of these variables are also snapshotted into the GameState struct for
 * netplay rollback (see game_state.h / game_state.c).
 *
...
u8 F_No2[2];
u8 F_No3[2];
s8 keep_condition[11];
s8 Check_Buff[4][2][12];
s8 Check_Convert[4][2][12];
...
u8 CPU_Rec[2];
u8 Pause_Type;
// u32 system_timer; // Removed duplicate
u16 Game_timer;
s16 Control_Time;
s16 Time_in_Time;
s16 Round_Level;
