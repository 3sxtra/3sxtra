...
void Lose_10000(PLW* wk) {
    ...
    } else if ((g_state.pcon_rno[1] == 0) || (g_state.pcon_rno[1] == 4)) {
        return;
    } else {
        switch (wk->wu.routine_no[3]) {
        case 0:
    ...
    if (set_field_adjust_flag(&g_state.plw[wk->wu.id], g_state.scrr, 1) != 0) {
...
void Lose_20000(PLW* wk) {
    ...
    case 0:
        ...
        if (!g_state.Extra_Break &&
            ((g_state.Round_num >= (CurrentSave()->Battle_Number[g_state.Play_Type]) * 2) ||
             (g_state.PL_Wins[g_state.Winner_id] >= (CurrentSave()->Battle_Number[g_state.Play_Type]) + 1))) {
