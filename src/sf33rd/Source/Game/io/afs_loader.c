...
#include "structs.h"
#include "sf33rd/Source/Game/io/afs_data.h"
...
s16 palette_request[PLAYER_COUNT];
u8 ldreq_break;
...
s32 Check_LDREQ_Queue_Player(s16 id) {
    ...
    if (id < 0 || id >= PLAYER_COUNT || palette_request[id] < 0 || palette_request[id] >= LDREQ_IX_SIZE) {
        return 0;
    }
    kara = ldreq_ix[palette_request[id]][0];
    made = kara + ldreq_ix[palette_request[id]][1];
...
/* ldreq_tbl[] and ldreq_ix[] data tables moved to afs_data.c */
