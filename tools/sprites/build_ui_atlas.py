...
def get_bank_file(output_dir, page, bank, w, h):
    if bank >= 32 or (w == 5 and h == 3):
        return os.path.join(output_dir, f"page_{page}_face_banks", f"face_bank_{bank:03d}.png")
        ...
        for cy in range(32):
            for cx in range(32):
                ...
                if target_bank is not None:
                    bank_file = get_bank_file(output_dir, page, target_bank, target_w, target_h)
