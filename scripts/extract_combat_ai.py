...
def extract_character(filepath):
    ...
    for m in PATTERN_FUNC_RE.finditer(content):
        ...
        for cm in CASE_CALL_RE.finditer(body):
            ...
            steps.append({"step": case_num, "op": func, "args": args})
    ...
    sorted_patterns = []
    for i in range(
        pattern_count if pattern_count > 0 else max(patterns.keys(), default=-1) + 1
    ):
        if i in patterns:
...
def extract_action_tables(base_dir):
    ...
    for i in range(5):
        ...
        for arr_match in re.finditer(
            r"const\s+u8\s+(\w+)\[(\d+)\]\[(\d+)\]\[(\d+)\]\s*=", content
        ):
            ...
            tables[name] = {"dimensions": [dim0, dim1, dim2], "data": data}
...
def main():
    ...
    for f in files:
        ...
        if result:
            ...
            print(
                f"  [{result['char_id']:2d}] {result['name']:10s}: {result['pattern_count']} patterns"
            )
    ...
    total_steps = sum(sum(len(p) for p in c["patterns"]) for c in characters.values())
    print(
